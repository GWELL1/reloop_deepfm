...
def _get_mindrecord_dataset(directory, train_mode=True, epochs=1, batch_size=1000,
    ...
    data_set = data_set.batch(int(batch_size / line_per_sample), drop_remainder=False)
    data_set = data_set.map(operations=(lambda x, y, z: (np.array(x).flatten().reshape(batch_size, 3),
                                                         np.array(y).flatten().reshape(batch_size, 3),
