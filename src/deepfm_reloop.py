...
class DenseLayer(nn.Cell):
    ...
    Args:
    """
    def __init__(self, input_dim, output_dim, weight_bias_init, act_str, keep_prob=1.0, convert_dtype=True,
                 use_act=True):
...
class NetWithLossClass(nn.Cell):
    def __init__(self, network, listds, steps_size, l2_coef=1e-6):
        ...
        self.relu = ops.ReLU()
        self.index = ms.Parameter([0], requires_grad=False)
    ...
    def construct(self, batch_ids, batch_wts, label):
        ...
        loss_reloop = alpha * lsc + (1 - alpha) * loss
        return loss_reloop
...
class ModelBuilder():
    ...
    def get_train_eval_net(self, listds, steps_size):     
        deepfm_net = DeepFMModel(self.model_config) 
        loss_net = NetWithLossClass(deepfm_net, listds, steps_size, l2_coef=self.train_config.l2_coef)
        train_net = TrainStepWrap(loss_net, lr=get_lr(self.train_config.learning_rate, self.train_config.train_epochs, steps_size),
